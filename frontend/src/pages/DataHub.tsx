    ...
    <div className="flex">
      <Sidebar>
        ...
        <button
          onClick={loadData}
          disabled={loading}
          className="w-full py-1.5 mt-2 mb-1 rounded text-[11px] font-semibold cursor-pointer border-none text-white"
                ...
                <div className="ml-3.5 mt-1">
                  {def.params.map((p) => (
                    <div key={p.name} className="flex items-center gap-1 mb-0.5">
                      <span className="text-[11px]" style={{ color: colors.dim, fontFamily: "var(--font-mono)", minWidth: 40 }}>
                        {p.label}
                      </span>
                      <input
        ...
        {addingIndicator ? (
          <select
            ...
            className="w-full rounded px-1.5 py-1 text-[11px] mb-1"
            style={inputStyle}
          >
          ...
          <button
            ...
            className="w-full py-1 rounded text-[11px] cursor-pointer border-none text-white mb-1"
        ...
        <button
          ...
          className="w-full py-1.5 rounded text-[11px] font-semibold cursor-pointer border-none text-white mb-1.5"
        ...
        <button
          ...
          disabled={crawling}
          className="w-full py-1.5 rounded text-[11px] font-semibold cursor-pointer border-none text-white"
          style={{ background: crawling ? "#444" : "#2A6040", fontFamily: "var(--font-mono)" }}
        >
          {crawling ? "Crawling…" : "Crawl Data"}
        </button>
        {crawl && (
          <div className="mt-1.5 text-[11px] p-1.5 rounded" style={{ background: colors.card, color: crawl.error ? colors.red : colors.green, fontFamily: "var(--font-mono)" }}>
      ...
      <div className="flex-1 p-3 overflow-y-auto" style={{ minWidth: 0 }}>
        ...
        >
          {coverage.length === 0 ? (
            <div className="text-[11px]" style={{ color: colors.dim, fontFamily: "var(--font-mono)" }}>
                  ...
                  <TableHeader>
                    <TableRow style={{ borderColor: colors.cardBorder }}>
                      {["Timestamp", "Open", "High", "Low", "Close", "Volume"].map((h) => (
                        <TableHead key={h} className="text-[11px] py-1 px-2" style={{ color: colors.dim, fontFamily: "var(--font-mono)" }}>{h}</TableHead>
                      ))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {bars.slice(-100).reverse().map((b, i) => (
                      <TableRow key={i} style={{ borderColor: colors.cardBorder }}>
                        <TableCell className="text-[11px] py-0.5 px-2" style={{ fontFamily: "var(--font-mono)", color: colors.muted }}>{b.timestamp.slice(0, 19)}</TableCell>
                        <TableCell className="text-[11px] py-0.5 px-2" style={{ fontFamily: "var(--font-mono)", color: colors.text }}>{b.open.toLocaleString()}</TableCell>
                        <TableCell className="text-[11px] py-0.5 px-2" style={{ fontFamily: "var(--font-mono)", color: colors.cyan }}>{b.high.toLocaleString()}</TableCell>
                        <TableCell className="text-[11px] py-0.5 px-2" style={{ fontFamily: "var(--font-mono)", color: colors.orange }}>{b.low.toLocaleString()}</TableCell>
                        <TableCell className="text-[11px] py-0.5 px-2" style={{ fontFamily: "var(--font-mono)", color: b.close >= b.open ? colors.green : colors.red }}>{b.close.toLocaleString()}</TableCell>
                        <TableCell className="text-[11px] py-0.5 px-2" style={{ fontFamily: "var(--font-mono)", color: colors.muted }}>{b.volume.toLocaleString()}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
