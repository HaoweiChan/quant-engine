"""Integration test for live execution disaster stop flow: entry fill -> register -> algo exit -> deregister."""
...
class TestLiveExecutionDisaster:
    @pytest.mark.asyncio
    async def test_entry_fill_register_algo_exit_deregister_flow(self) -> None:
        ...
        results = await live_engine.execute(entry_orders, snapshot)

        assert len(results) == 1
        assert results[0].status == "filled"
        assert live_engine._active_disaster_stops == 1
