...
def _make_snapshot(price: float, ts: datetime | None = None) -> MarketSnapshot:
    return MarketSnapshot(
        price=price, atr={"daily": 100.0},
        timestamp=ts or datetime.now(UTC),
        margin_per_unit=184000.0, point_value=200.0,
        min_lot=1.0, contract_specs=_make_specs(),
    )


def _make_signal(
    direction: float = 1.0, conf: float = 0.8,
) -> MarketSignal:
    return MarketSignal(
        timestamp=datetime.now(UTC), direction=direction, direction_conf=conf,
        regime="trending", trend_strength=0.7, vol_forecast=120.0,
        suggested_stop_atr_mult=None, suggested_add_atr_mult=None,
        model_version="test-v1", confidence_valid=True,
    )
