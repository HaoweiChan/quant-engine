...
A unified, read-only broker abstraction layer that allows the dashboard and trading sessions to query account state (equity, positions, margin, fills) across different brokers through a single interface. All broker-specific logic is isolated in `BrokerGateway` subclasses; the rest of the system only deals with `AccountSnapshot`.
