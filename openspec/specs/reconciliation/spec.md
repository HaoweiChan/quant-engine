...
Periodically compare the engine's internal position and account state against the broker's reported state. Detect and alert on mismatches to ensure the system's view of reality stays accurate.
...
- **THEN** the reconciler SHALL log an error and dispatch an urgent Telegram alert — this indicates a fill was missed or the engine state is stale

#### Scenario: Orphan position (broker-only)
- **WHEN** the broker reports a position that the engine does not track
- **THEN** the reconciler SHALL log an error and dispatch an urgent Telegram alert — this indicates a manual trade or an untracked fill
