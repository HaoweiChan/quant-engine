...
An embedded Ace code editor in the Strategy tab that allows users to view and edit their strategy files (`src/strategies/`) directly from the dashboard without switching to an IDE. Provides three-level validation on save (syntax, lint, engine) and tracks file modifications to signal stale backtest results. Core system files (`src/core/`, `src/bar_simulator/`) are intentionally excluded — users have access only to strategy-level code.
...
The editor SHALL track which files have been saved since the last backtest run. When one or more strategy files have been modified, a visual indicator SHALL appear signaling that backtest results may be stale.

#### Scenario: Stale indicator appears after save
- **WHEN** the user saves a file in the Strategy editor
- **THEN** a "files modified — re-run backtest" indicator SHALL appear until the next backtest is run

#### Scenario: Indicator clears after backtest run
- **WHEN** the user runs a backtest after modifying files
- **THEN** the stale indicator SHALL be removed
...
After saving a strategy `.py` file, the editor SHALL attempt to import the user strategy modules and instantiate a `PositionEngine` with them. If instantiation succeeds, "Engine OK" SHALL appear. If it fails, the error SHALL be displayed.
...
- **THEN** engine validation SHALL be skipped — TOML files receive only a "Saved" confirmation
