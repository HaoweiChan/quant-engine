...
- **THEN** credentials SHALL be encrypted via `CredentialStore` and persisted to `trading.db`, and a success toast SHALL appear

#### Scenario: Reconnect tests connection
- **WHEN** the user clicks "Reconnect"
- **THEN** the system SHALL attempt to connect using the current credentials, showing a "Connecting..." spinner, then "Connected" (green) or "Failed: {error}" (red)
...
The Trading primary tab SHALL display a War Room sub-tab as its default view. The War Room SHALL present a multi-panel layout showing all connected accounts and their active strategy sessions in a single view.

#### Scenario: War Room loads on Trading tab click
- **WHEN** the user clicks the Trading primary tab
- **THEN** the War Room sub-tab SHALL be active by default, showing the account overview and session monitors
