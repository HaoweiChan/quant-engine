...
The registry SHALL expose `get_active_params(slug: str) -> dict[str, Any]` that returns the effective parameter values: TOML overrides from `configs/<slug>.toml` merged over `PARAM_SCHEMA` defaults.

#### Scenario: No TOML override exists
- **WHEN** `get_active_params("atr_mean_reversion")` is called and `configs/atr_mean_reversion.toml` does not exist
- **THEN** it SHALL return the `"default"` value from each entry in `PARAM_SCHEMA`

#### Scenario: TOML override partially overrides defaults
- **WHEN** `configs/atr_mean_reversion.toml` contains `bb_len = 20` but no other keys
- **THEN** `get_active_params("atr_mean_reversion")` SHALL return `bb_len: 20` with all other keys from `PARAM_SCHEMA` defaults
